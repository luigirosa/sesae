		...
		switch ($classe) {
			...
			case CH_STATGEN:
				...
				$r = $db->query("SELECT COUNT(targetdata.goog_analytics)
                         FROM target 
	                       JOIN targetdata ON targetdata.idtarget=target.idtarget
		                     WHERE targetdata.goog_analytics<>'' $wha")->fetch_array();
				$tuttig = $r[0];
				...
				$r = $db->query("SELECT COUNT(targetdata.goog_tag)
                         FROM target 
	                       JOIN targetdata ON targetdata.idtarget=target.idtarget
		                     WHERE targetdata.goog_tag<>'' $wha")->fetch_array();
				$tuttig = $r[0];
				...
				$r = $db->query("SELECT COUNT(targetdata.goog_asy)
                         FROM target 
	                       JOIN targetdata ON targetdata.idtarget=target.idtarget
		                     WHERE targetdata.goog_asy<>'' $wha")->fetch_array();
				$tuttig = $r[0];
			...
			case CH_POWEREDBY:
				...
				while ($r = $q->fetch_array()) {
					$b .= "\n<tr><td align='left' style='text-align: left;'>$r[poweredby_stat_fam]</td><td align='right' style='text-align: right;'>" . number_format($r['c'], 0, ',', '.') . "</td><td align='right' style='text-align: right;'>" . number_format(($r['c']*100/$t[0]), 2, ',', '.') . "%</td></tr>";
				}
				$b.= "\n</table>";
